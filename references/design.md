...
先用文字展示产出路径 + 设计要点摘要（3–5 条）+ 找到的主要风险点。随后**调用 `AskUserQuestion` 工具**给出可点选的下一步（约定见 SKILL.md「交互约定」，禁止只用文字罗列），选项对应：

- 进入 **review** 阶段做设计评审（**流程要求**——设计必须评审通过才能进入编码）——默认首选项
- 先修改设计

**编码门禁**：设计文档必须经 review 阶段评审且结论为「通过」后才能进入 code。因此选项里**不列**「进入 code」，也不提供"跳过评审直接开发"选项；用户如坚持跳过，须经自由输入（Other）显式声明知道风险，并在评审/实现产出中留痕。
