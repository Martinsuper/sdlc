...
def init(path, depth, no_llm, force, template, adapter, no_commit, interactive):
    ...
    # Create .sdlc/config.toml with auto-detected info
    ...
    if not config_path.exists() or force:
        ...
        config_lines = ["# SDLC project configuration", "# Generated by sdlc init", ""]
        config_lines.append(f'project_name = "{config_data["project"]["name"]}"')
        config_lines.append(f'language = "{config_data["project"]["language"]}"')
        config_lines.append(f'framework = "{config_data["project"]["framework"]}"')
        config_lines.append(f'adapter = "{config_data["adapter"]}"')
        config_lines.append(f'profile = "{config_data["profile"]}"')
        tech_stack_str = ", ".join(config_data["project"]["tech_stack"])
        config_lines.append(f'tech_stack = ["{tech_stack_str}"]' if tech_stack_str else "tech_stack = []")
